-- Only apply RLS to tables that don't have it yet
-- This avoids conflicts with existing policies

-- First, check which tables need RLS
SELECT 
    'TABLES WITHOUT RLS:' as info,
    tablename
FROM pg_tables
...
-- ============================================
-- Only process tables that don't have RLS
-- ============================================
...
-- Show final status
SELECT 
    'FINAL STATUS - Tables still without RLS:' as info,
    tablename
FROM pg_tables
WHERE schemaname = 'public'
AND rowsecurity = false
AND tablename NOT LIKE '%_analytics%'  -- Skip analytics tables
AND tablename NOT IN (
    'schema_migrations',  -- Skip system tables
    'spatial_ref_sys'
)
ORDER BY tablename;