-- COMPLETE SIZING SYSTEM INSTALLATION AND VARIANT GENERATION
-- This script will install the sizing system and create variants for existing products

-- STEP 1: Install the size templates (re-run to ensure they exist)
...
-- STEP 2: Generate variants for SUITS (all products with "suit" in name or category)
INSERT INTO public.product_variants (product_id, sku, size_display, option1, inventory_quantity, stock_quantity, price, status)
SELECT 
    p.id as product_id,
    p.id::text || '-' || sizes.size as sku,
    sizes.size as size_display,
...
-- STEP 3: Generate variants for BLAZERS (products with "blazer" in name or category)
INSERT INTO public.product_variants (product_id, sku, size_display, option1, inventory_quantity, stock_quantity, price, status)
SELECT 
    p.id as product_id,
    p.id::text || '-' || sizes.size as sku,
...
-- STEP 4: Generate variants for DRESS SHIRTS
INSERT INTO public.product_variants (product_id, sku, size_display, option1, option2, inventory_quantity, stock_quantity, price, status)
SELECT 
    p.id as product_id,
    p.id::text || '-' || neck || '-' || sleeve || '-' || fit as sku,
    neck || '/' || sleeve as size_display,
    neck || '/' || sleeve as option1,
    fit as option2,
    10 as inventory_quantity,
    10 as stock_quantity,
    p.base_price as price,
...
CROSS JOIN (
    SELECT 
        unnest(ARRAY['15', '15.5', '16', '16.5', '17', '17.5', '18']) as neck,
        unnest(ARRAY['32-33', '34-35', '36-37']) as sleeve,
        unnest(ARRAY['slim_cut', 'classic_fit']) as fit
) sizes
WHERE (p.name ILIKE '%shirt%' OR p.category ILIKE '%shirt%')
AND NOT EXISTS (
    SELECT 1 FROM public.product_variants pv 
    WHERE pv.product_id = p.id AND pv.option1 = (neck || '/' || sleeve)
);

-- STEP 5: Generate variants for SWEATERS/CASUAL (everything else)
INSERT INTO public.product_variants (product_id, sku, size_display, option1, inventory_quantity, stock_quantity, price, status)
SELECT 
    p.id as product_id,
    p.id::text || '-' || sizes.size as sku,
...
FROM public.products p
CROSS JOIN (
    SELECT unnest(ARRAY['S', 'M', 'L', 'XL', '2XL', '3XL']) as size
) sizes
...
-- STEP 6: Update total_inventory for all products
...
SELECT 
    ...
    STRING_AGG(DISTINCT pv.option1, ', ') as sizes_created
FROM public.products p
